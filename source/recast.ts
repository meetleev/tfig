...
export function recastTopLevelModule({
    ...
    function tryEmplaceModifier(modifiers: ts.Modifier[], kind: ts.Modifier['kind']) {
        if (modifiers.every((m: ts.Modifier) => m.kind !== kind)) {
            ...
            if (interop.imports.length !== 0) {
                statements.push(nodeFactor.createImportDeclaration(
                    ...
                    nodeFactor.createImportClause(
                        ...
                        nodeFactor.createNamedImports(interop.imports.map(({
                                                                               importName,
                                                                               asName
                                                                           }) => asName === importName ?
    ...
    function recastNamespaceTraits(namespaceTraits: rConcepts.NamespaceTraits) {
        ...
        if (namespaceTraits.selfExports.length !== 0) {
            statements.push(nodeFactor.createExportDeclaration(
                undefined, // modifiers
                false, // isTypeOnly
                nodeFactor.createNamedExports(namespaceTraits.selfExports.map(({
                                                                                   importName,
                                                                                   asName
                                                                               }) => asName === importName ?
                    nodeFactor.createExportSpecifier(false, undefined, nodeFactor.createIdentifier(importName)) :
        ...
        if (ts.isClassDeclaration(statement)) {
            ...
            return copyComments(statement, nodeFactor.createVariableStatement(
                ...
                nodeFactor.createVariableDeclarationList(
                    statement.declarationList.declarations.map((declaration: ts.VariableDeclaration) =>
    ...
    function recastModuleDeclarationAsNamespaceDeclaration(moduleDeclaration: ts.ModuleDeclaration, newName: string) {
        ...
        if (!body) {
            ...
            newBody = nodeFactor.createModuleBlock(recastStatements((body as ts.ModuleBlock).statements));
        } else {
            console.warn(`Unknown module declaration type ${tsUtils.stringifyNode(body as ts.Node)}`);
        }
    ...
    function recastModifiers(modifiers: ts.NodeArray<ts.Modifier> | undefined): ts.Modifier[];
    ...
    function recastTypeNode(type: ts.TypeNode | undefined): ts.TypeNode  ;
    function recastTypeNode(type: ts.TypeNode | undefined): ts.TypeNode | undefined {
        if (!type) {
            return undefined;
        }
        ...
        if (ts.isTypeReferenceNode(type)) {
            return recastEntityNameAsTypeNode(
                type.typeName,
                type.typeArguments ? type.typeArguments.map((ta: ts.TypeNode) => recastTypeNode(ta)) : undefined,
            );
